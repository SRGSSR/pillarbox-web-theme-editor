...
describe('UiState', () => {
  ...
  it('falls back to the defaults when the stored value is corrupted', () => {
    localStorage.setItem('pbte-ui-state', '{not json');

    expect(UiState.load().previewDock).toBe('bottom');
  });

  it('can save over a corrupted stored value', () => {
    localStorage.setItem('pbte-ui-state', '{not json');
    UiState.save({ previewDock: 'right' });

    expect(UiState.load().previewDock).toBe('right');
  });

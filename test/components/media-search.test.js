...
describe('MediaSearch', () => {
  ...
  it('dispatches media-selected when a result is activated with the keyboard', async() => {
    let detail;

    element.addEventListener('media-selected', (e) => (detail = e.detail));
    await runSearch('federer');

    const result = element.shadowRoot.querySelector('[part~="result"]');

    result.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter' }));
  ...
  it('keeps the dropdown open when clicking inside the component', async() => {
    ...
    element.shadowRoot.querySelector('[part~="input"]').click();
    await element.updateComplete;

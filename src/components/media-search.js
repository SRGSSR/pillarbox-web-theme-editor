...
class MediaSearch extends LitElement {
  ...
  /**
   * Closes the results dropdown on clicks outside the component; bound once so
   * the document listener can be removed on disconnect.
   *
  ...
  #renderResult(result) {
    return html`
      <li part="result"
          ...
          @keydown="${(e) => this.#handleResultKeydown(e, result)}">
  ...
  /**
   * Selects a result from the keyboard, mirroring the activation behaviour a
   * native button would provide.
   *
   * @param {KeyboardEvent} e The keydown event on the result.
   * @param {Object} result The media result.
   * @private
   */
  #handleResultKeydown(e, result) {
